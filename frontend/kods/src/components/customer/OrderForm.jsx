...
export default function OrderForm() {
  ...
  const updateRow = (index, field, value) => {
    const updatedOrders = fishOrders.map((order, i) =>
      i === index ? { ...order, [field]: value, total: (field === 'quantity' ? value : order.quantity) * (field === 'price' ? value : order.price) } : order
    );
    setFishOrders(updatedOrders);
  };
...
>>>>>>> main:frontend/kods/src/components/customer/OrderForm.jsx
        ...
        <div className="sender-info">
          ...
          <input require className="input-customer" type="text" placeholder="Name" onChange={(e) => handleCustomerChange('nameSender', e.target.value)} />
          <input className="input-customer" type="text" placeholder="Phone" onChange={(e) => handleCustomerChange('phoneSender', e.target.value)} />
          <input className="input-customer" type="text" placeholder="Address" onChange={(e) => handleCustomerChange('addressSender', e.target.value)} />
        </div>

        <div className="customer-info">
          <h3 className="label-customer">Customer Information</h3>
          <input className="input-customer" type="text" placeholder="Name" onChange={(e) => handleCustomerChange('nameCustomer', e.target.value)} />
          <input className="input-customer" type="text" placeholder="Phone" onChange={(e) => handleCustomerChange('phoneCustomer', e.target.value)} />
          <input className="input-customer" type="text" placeholder="Address" onChange={(e) => handleCustomerChange('addressCustomer', e.target.value)} />
          <input className="input-customer" type="number" placeholder="Distance (km)" min="0" onChange={(e) => handleCustomerChange('distance', parseInt(e.target.value) || 0)} />
        </div>
            ...
            <div className="container-popup">
              ...
              <div className="layout-popup">
                <div className="layout-qrcode">
                  ...
                  <QRCodeSVG value="https://your-payment-url-1.com" size={256} />
                </div>
                <div className="layout-qrcode">
                  <p className="label-qrcode">Payment via Momo</p>
                  <QRCodeSVG value="https://your-payment-url-2.com" size={256} />
                </div>
              </div>
